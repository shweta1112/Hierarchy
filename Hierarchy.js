...
getHierarchy = () => {
  var output = [];
  var resultObject = {};
  var rootNode = payBeneficiaries.filter(r => r.parentBenePayId === "")[0];
  resultObject.orgHierarchy = [rootNode.beneficiary.businessName];
  output.push(Object.assign({}, resultObject));
  var level1Nodes = payBeneficiaries.filter(
    r => r.parentBenePayId === rootNode.beneficiaryPayId
  );

  level1Nodes
    .map(s => [rootNode.beneficiary.businessName, s.beneficiary.businessName])
    .map(x => {
      resultObject.orgHierarchy = x;
      output.push(Object.assign({}, resultObject));
      return x;
    });
  level1Nodes
    .flatMap(rr =>
      payBeneficiaries
        .filter(ss => ss.parentBenePayId === rr.beneficiaryPayId)
        .map(xx => [
          rootNode.beneficiary.businessName,
          rr.beneficiary.businessName,
          xx.beneficiary.businessName
        ])
    )
    .map(a => {
      resultObject.orgHierarchy = a;
      output.push(Object.assign({}, resultObject));
    });
  console.log(output);
};
...
var output1 = [
  ...
  },
  {
    orgHierarchy: [
      "Royal NNJ p.l.c.",
      "Torphy-Osinski",
      "Nikolaus Dickinson and Wiegand"
    ]
  },
  {
    orgHierarchy: [
      "Royal NNJ p.l.c.",
      "Hammes Macejkovic and Steuber",
      "Rath Group"
    ]
  },
  {
